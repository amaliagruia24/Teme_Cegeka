...
{
    class RentalCarsBucharest : RentalCars
    {
        public RentalCarsBucharest(string name) : base(name)
        {
        }
        ...
        public override double GetAmountFromCustomer(Rental rental)
        {
            ...
            switch (rental.Car.PriceCode)
            {
                case PriceCode.Regular:
                    Regular regular = new Regular(Constants.RegularPricePerDayBucharest,
                ...
                case PriceCode.Luxury:
                    Luxury luxury = new Luxury(Constants.LuxuryPricePerDay);

                    if (rental.Customer.FrequentRenterPoints >= Constants.LuxuryRenterPoints)
                    {
                        thisAmount = luxury.CaculateAmount(rental.DaysRented);
                    }
